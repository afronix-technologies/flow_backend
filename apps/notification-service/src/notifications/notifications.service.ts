...
export class NotificationsService {
    ...
    async handleEmailJob(job: Job) {
        ...
        try {
            ...
            if (type === 'verify') await this.emailService.sendVerificationEmail(payload.email, payload.token);
            else if (type === 'invite') await this.emailService.sendInvitationEmail(payload.email, payload.organizationName, payload.token);
            // ... Handle other types
