...
export class EmailService {
    ...
    constructor() {
        ...
        if (process.env.SMTP_HOST) {
            ...
            this.logger.warn('SMTP configuration not found. Email service operating in mock mode (logging only).');
        }
    }

    private async sendMail(to: string, subject: string, html: string) {
        if (!this.transporter) {
            this.logger.log(`[MOCK EMAIL] To: ${to} | Subject: ${subject} | Content: ${html.substring(0, 50)}...`);
            return;
        }
