...
export class InitialMigration1768597667003 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TYPE "public"."auth_invitations_role_enum" AS ENUM('owner', 'admin', 'manager', 'user')`);
        await queryRunner.query(`CREATE TYPE "public"."auth_invitations_status_enum" AS ENUM('pending', 'accepted', 'expired', 'cancelled')`);
        ...
        await queryRunner.query(`CREATE TYPE "public"."user_organizations_role_enum" AS ENUM('owner', 'admin', 'manager', 'user')`);
        await queryRunner.query(`CREATE TABLE "user_organizations" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "user_id" uuid NOT NULL, "organization_id" uuid NOT NULL, "role" "public"."user_organizations_role_enum" NOT NULL DEFAULT 'user', "joined_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_51ed3f60fdf013ee5041d2d4d3d" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE UNIQUE INDEX "IDX_f143fa57706c0fb840301ad704" ON "user_organizations" ("user_id", "organization_id") `);
        await queryRunner.query(`CREATE TYPE "public"."organizations_subscription_status_enum" AS ENUM('active', 'trial', 'expired', 'cancelled')`);
        await queryRunner.query(`CREATE TYPE "public"."organizations_payment_cycle_enum" AS ENUM('monthly', 'quarterly', 'annual')`);
        await queryRunner.query(`CREATE TYPE "public"."organizations_onboarding_step_enum" AS ENUM('registered', 'email_verified', 'team_details', 'template_selected', 'completed')`);
        ...
        await queryRunner.query(`CREATE TYPE "public"."auth_users_role_enum" AS ENUM('owner', 'admin', 'manager', 'user')`);
        ...
        await queryRunner.query(`CREATE UNIQUE INDEX "IDX_b5b9d9e0b4721a1b5aeac416b7" ON "auth_users" ("email", "organization_id") `);
        ...
        await queryRunner.query(`CREATE UNIQUE INDEX "IDX_9a99b8526fd318f63477dcfccd" ON "auth_sessions" ("token") `);
        await queryRunner.query(`CREATE TABLE "auth_roles" ("id" SERIAL NOT NULL, "name" character varying NOT NULL, "description" character varying, "permissions" text, "isActive" boolean NOT NULL DEFAULT true, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "UQ_6e74f330e34555ae90068b03928" UNIQUE ("name"), CONSTRAINT "PK_fa9e7a265809eafa9e1f47122e2" PRIMARY KEY ("id"))`);
        await queryRunner.query(`ALTER TABLE "auth_invitations" ADD CONSTRAINT "FK_a36e78cf70fd1fa2b0b57e479ea" FOREIGN KEY ("organization_id") REFERENCES "organizations"("id") ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "auth_invitations" ADD CONSTRAINT "FK_a4198988451c0a8042ca88b7453" FOREIGN KEY ("invited_by_user_id") REFERENCES "auth_users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "user_organizations" ADD CONSTRAINT "FK_6881b23cd1a8924e4bf61515fbb" FOREIGN KEY ("user_id") REFERENCES "auth_users"("id") ON DELETE CASCADE ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE "auth_users" ADD CONSTRAINT "FK_d0b3b7c01c35a40e83f3a4bfe82" FOREIGN KEY ("organization_id") REFERENCES "organizations"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "auth_sessions" ADD CONSTRAINT "FK_50ccaa6440288a06f0ba693ccc6" FOREIGN KEY ("user_id") REFERENCES "auth_users"("id") ON DELETE CASCADE ON UPDATE NO ACTION`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE "auth_sessions" DROP CONSTRAINT "FK_50ccaa6440288a06f0ba693ccc6"`);
        await queryRunner.query(`ALTER TABLE "auth_users" DROP CONSTRAINT "FK_d0b3b7c01c35a40e83f3a4bfe82"`);
        ...
        await queryRunner.query(`ALTER TABLE "auth_invitations" DROP CONSTRAINT "FK_a4198988451c0a8042ca88b7453"`);
        await queryRunner.query(`ALTER TABLE "auth_invitations" DROP CONSTRAINT "FK_a36e78cf70fd1fa2b0b57e479ea"`);
