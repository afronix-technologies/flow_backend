...
export class AuthService {
    ...
    async register(registerDto: RegisterDto): Promise<{ message: string }> {
        ...
        if (existingUser) {
            ...
            throw new ConflictException('User with this email already exists and is verified. Please login.');
        }
    ...
    async login(loginDto: LoginDto): Promise<AuthResponseDto | { organizations: Organization[] }> {
        ...
        if (users.length > 1 && !loginDto.organizationId) {
            ...
            if (!userToLogin) throw new UnauthorizedException('Invalid credentials for this organization');
        } else {
            // Single user
            userToLogin = users[0];
        }
        ...
        const isPasswordValid = await this.passwordService.compare(loginDto.password, userToLogin.password);
    ...
    async inviteUser(inviterId: string, inviteDto: InviteUserDto): Promise<{ message: string }> {
        ...
        // Get Org Name
        const org = await this.organizationRepository.findOne({ where: { id: inviter.organizationId } });
    ...
    async bulkInvite(inviterId: string, bulkInviteDto: BulkInviteDto): Promise<{ message: string, sentCount: number }> {
        const inviter = await this.userRepository.findOne({ where: { id: inviterId } });
        if (!inviter) throw new UnauthorizedException();
        ...
        for (const invite of bulkInviteDto.invitations) {
            ...
            const existingInvite = await this.invitationRepository.findOne({
                where: { email: invite.email, organizationId: inviter.organizationId, status: InvitationStatus.PENDING }
            });
    ...
    async validateJoinLink(token: string): Promise<{ organizationName: string, isValid: boolean }> {
        ...
        if (!org || !org.joinLinkEnabled || (org.joinLinkExpiresAt && org.joinLinkExpiresAt < new Date())) {
    ...
    async joinViaLink(token: string, userData: { email: string, password?: string, firstName: string, lastName: string }): Promise<AuthResponseDto> {
        const org = await this.organizationRepository.findOne({ where: { joinLinkToken: token } });
    ...
    async resetPassword(resetDto: ResetPasswordDto): Promise<{ message: string }> {
        const user = await this.userRepository.findOne({ where: { passwordResetToken: resetDto.token } });
