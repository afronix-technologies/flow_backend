...
export class EmailService {
    ...
    async sendVerificationEmail(email: string, token: string) {
        try {
            await axios.post(`${this.notificationServiceUrl}/email/verify`, { email, token });
    ...
    async sendInvitationEmail(email: string, organizationName: string, token: string) {
        try {
            await axios.post(`${this.notificationServiceUrl}/email/invite`, { email, organizationName, token });
    ...
    async sendPasswordResetEmail(email: string, token: string) {
        try {
            await axios.post(`${this.notificationServiceUrl}/email/reset-password`, { email, token });
    ...
    async sendWelcomeEmail(email: string, firstName: string) {
        try {
            await axios.post(`${this.notificationServiceUrl}/email/welcome`, { email, firstName });
